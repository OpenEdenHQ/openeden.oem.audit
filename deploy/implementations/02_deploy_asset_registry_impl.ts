...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log('🚀 Deploying AssetRegistry Implementation (for manual upgrade)');
  console.log('📌 Deployer address:', deployer);

  // Check if already deployed
  try {
    const existing = await get('AssetRegistryImpl');
    console.log('📌 AssetRegistry implementation already deployed at:', existing.address);
  ...
  // Deploy AssetRegistry Implementation (no proxy, just the implementation)
  console.log('\n1️⃣ Deploying AssetRegistry implementation...');
  const AssetRegistry = await ethers.getContractFactory('AssetRegistry');
  const assetRegistryImpl = await AssetRegistry.deploy();
  await assetRegistryImpl.waitForDeployment();
  const assetRegistryImplAddress = await assetRegistryImpl.getAddress();
  console.log('✅ AssetRegistry implementation deployed to:', assetRegistryImplAddress);

  // Save deployment info
  await deployerDeployments.save('AssetRegistryImpl', {
  ...
  // Summary
  console.log('\n📋 Deployment Summary:');
  console.log('AssetRegistry Implementation:', assetRegistryImplAddress);
  ...
  if (network.name !== 'hardhat' && network.name !== 'localhost') {
    ...
    try {
      ...
      console.log('✅ AssetRegistry implementation verified on Etherscan');
    } catch (error: any) {
  ...
  console.log('   4. Enter the new implementation address:', assetRegistryImplAddress);
  console.log('   5. Confirm the transaction');
};

func.tags = ['asset_registry_impl', 'implementations'];
export default func;
